...
export default async function handler(req, res) {
  ...
  try {
    if (accion === 'confirmado') {
      ...
      await notificarRechazado({ telefono, nombre, peluqueria_nombre, motivo, peluqueria_id })
    } else if (accion === 'modificado') {
      await notificarModificado({
        telefono, nombre, peluquero_nombre,
        fecha_propuesta, hora_propuesta,
        peluqueria_id,
      })
    } else if (accion === 'cancelado') {
      await notificarCancelado({ telefono, nombre, peluqueria_nombre, motivo, peluqueria_id })
